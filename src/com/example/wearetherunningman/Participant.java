...
public class Participant extends FragmentActivity {
	...
	public class asyncTaskMarker extends
			AsyncTask<String, ArrayList<String[]>, String> implements
				...
				for (int i = 0; i < fetchRows.size(); i++) {
					...
					if (rows != null) {
						if (team.equals(rows[2])) { // 나와 같은팀이면
							...
							float[] distance = new float[2]; // float 형태의 사이즈 2의
																// 행렬 생성
							float actual_distance; // 실제 거리 값을 담을 변수

							// LatLng myloc=player.

							Location.distanceBetween(player.latitude / 1E6,
									player.longitude / 1E6,
									Double.parseDouble(rows[3]) / 1E6,
									Double.parseDouble(rows[4]) / 1E6, distance);

							// lat1와 lon1은 첫번째 사용자, lat2와 lon2는 두번째 사용자의 GPS 값.
							// distanceBetween은 Location클래스 내에서 정의된 static 함수이기
							// 때문에 Location 클래스를 통해 아무데서나 부를 수 있다.
							// 이 메소드가 호출되고 나면 distance 행렬의 첫번째 요소로 두 지점의 거리가
							// 할당된다.

							actual_distance = distance[0] * 0.000621371192f; // 간단한
																				// 사용을
																				// 위해
																				// 일반
																				// 변수로
